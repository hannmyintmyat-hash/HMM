...
These errors and others that are due to overload will have an [`.overloaded` property](/durable-objects/features/error-handling) set on their exceptions, which can be used to avoid retrying overloaded operations.
