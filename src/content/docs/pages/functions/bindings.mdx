...
To interact with a D1 database via the Wrangler CLI while [developing locally](/d1/features/local-development/#develop-locally-with-pages), add `--d1 <BINDING_NAME>=<DATABASE_ID>` to the `wrangler pages dev` command.
